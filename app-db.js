...
const APP_VERSION='2.9.77-validation-module-fix';
...
CollectorApp.services.selectSurveyForModule=(...args)=>selectSurveyForModule(...args);
...
function selectSurveyForModule(module){const target=module==='livestock'?'livestock':module==='crop'?'crop':null;if(!target)return false;const current=getSelectedSurveyConfig();if(current&&(current.survey_type===target||current.survey_type==='mixed'))return true;const candidates=normalizeSurveyCatalog(Object.values(availableSurveyConfigs));const match=candidates.find(item=>item.is_active!==false&&(item.survey_type===target||item.survey_type==='mixed'))||candidates.find(item=>item.survey_type===target||item.survey_type==='mixed');if(!match?.name)return false;const source=document.getElementById('survey-name');const mirror=document.getElementById('collector-survey-name');[source,mirror].forEach(select=>{if(!select)return;if(![...select.options].some(option=>option.value===match.name)){const option=document.createElement('option');option.value=match.name;option.textContent=match.name;option.dataset.surveyType=match.survey_type||'';select.appendChild(option)}select.value=match.name});rememberSurveyName(match.name);window.syncCollectorSurveyMenu?.();window.refreshCollectorSurveyTypeMatch?.();return true}
...
function getValidationModule(){const active=String(window.activeCollectorPrimaryModule||document.body.dataset.collectorPrimaryModule||window.CropCollectorApp?.state?.get?.('activeCollectorPrimaryModule')||'').toLowerCase();if(active==='livestock')return 'livestock';if(active==='crop')return 'crop';const selected=getSelectedSurveyConfig();return selected?.survey_type==='livestock'?'livestock':'crop'}
function syncValidationModuleUi(){const module=getValidationModule();const livestock=module==='livestock';if(livestock&&validationGeometryFilter!=='point')validationGeometryFilter='point';if(!livestock&&(validationGeometryFilter==='all'||validationGeometryFilter===''))validationGeometryFilter='polygon';const polygonPick=document.getElementById('validate-pick-polygon');const pointPick=document.getElementById('validate-pick-point');const allFilter=document.getElementById('validate-filter-all');const polygonFilter=document.getElementById('validate-filter-polygon');const pointFilter=document.getElementById('validate-filter-point');const target=document.getElementById('validation-target');[polygonPick,polygonFilter,allFilter].forEach(node=>{if(!node)return;node.hidden=livestock;node.style.display=livestock?'none':''});if(pointPick){pointPick.innerHTML=livestock?'<i data-feather="map-pin"></i> Select Livestock Point':'<i data-feather="map-pin"></i> Select Point';pointPick.className=livestock?'':'secondary'}if(polygonPick){polygonPick.innerHTML='<i data-feather="hexagon"></i> Select Crop Polygon'}if(pointFilter){pointFilter.textContent=livestock?'Livestock Points':'Points Only';pointFilter.className=livestock?'':'secondary'}if(polygonFilter)polygonFilter.textContent='Crop Polygons';if(allFilter)allFilter.textContent='All Features';if(target&&!selectedEntryId)target.textContent=livestock?'Select a synced livestock point record to validate.':'Select a synced crop polygon to validate.';feather.replace()}
function updateValidationGeometryButtons(){const allBtn=document.getElementById('validate-filter-all'),polygonBtn=document.getElementById('validate-filter-polygon'),pointBtn=document.getElementById('validate-filter-point'),status=document.getElementById('validation-filter-status');syncValidationModuleUi();if(allBtn){allBtn.className=validationGeometryFilter==='all'?'':'secondary'}if(polygonBtn){polygonBtn.className=validationGeometryFilter==='polygon'?'':'secondary'}if(pointBtn){pointBtn.className=validationGeometryFilter==='point'?'':'secondary'}if(status)status.textContent=`Validation filter: ${getValidationGeometryLabel()}.`;feather.replace()}
function setValidationGeometryFilter(filter){const module=getValidationModule?.()||'crop';validationGeometryFilter=module==='livestock'?'point':(filter||'polygon');selectedEntryId=null;updateValidationGeometryButtons();updateValidationTarget();if(getActiveMode()==='validate')renderEntries()}
...
function beginValidationSelection(filter){if(!hasRole('validator','admin')){setValidationSelectionMessage('Your role cannot access validation mode.','warning');showCollectorFeedback('Validation tools are available only to validators and admins.','warning',{sticky:true});return}const module=getValidationModule?.()||'crop';const effectiveFilter=module==='livestock'?'point':(filter||'polygon');const validateSection=document.getElementById('validate-form')?.closest('.section');setMode('validate');setValidationGeometryFilter(effectiveFilter);toggleSidebar(true);collapseSidebarSectionsExcept(document.getElementById('collector-quick-menu'),validateSection);openSidebarSection(validateSection);setQuickMenuActive('validate');const candidate=findNearestValidationEntry(effectiveFilter);if(candidate){focusEntry(candidate.id||candidate.localId);setValidationSelectionMessage(`Nearest ${module==='livestock'?'livestock point':effectiveFilter} selected automatically.`, 'info')}else{setValidationSelectionMessage(`No synced ${module==='livestock'?'livestock points':effectiveFilter+'s'} are available for validation right now.`, 'warning')}const target=document.getElementById('validation-target');if(target)target.scrollIntoView({behavior:'smooth',block:'nearest'})}
...
Object.assign(window,{selectSurveyForModule,syncValidationModuleUi});
CollectorApp.registerModule('db',{
  ...
  selectSurveyForModule,
  syncValidationModuleUi,
  applyAppVersionMigrations,
  registerAppServiceWorker,
  enforceProvisionedAccessUI
});
