...
function normalizeSurveyCatalog(items=[]){return(Array.isArray(items)?items:[]).map(item=>({name:String(item?.name||'').trim(),description:item?.description||null,is_active:item?.is_active!==false,survey_type:item?.survey_type==='livestock'?'livestock':'crop',enabled_modules:Array.isArray(item?.enabled_modules)?item.enabled_modules.filter(Boolean):[],enabled_question_steps:Array.isArray(item?.enabled_question_steps)?item.enabled_question_steps.filter(Boolean):[]})).filter(item=>item.name)}
